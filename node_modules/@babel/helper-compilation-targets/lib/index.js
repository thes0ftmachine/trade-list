...
import { isValid, isLess, parse, isGreater } from 'verkit';
...
function getInclusionReasons(item, targetVersions, list) {
  ...
  return Object.keys(targetVersions).reduce((result, env) => {
    ...
    if (!minVersion) {
      ...
      if (!targetIsUnreleased && (minIsUnreleased || isLess(targetVersion.toString(), semverify(minVersion)))) {
        result[env] = prettifyVersion(targetVersion);
      }
    }
    return result;
...
function targetsSupported(target, support) {
  ...
  const unsupportedEnvironments = targetEnvironments.filter(environment => {
    ...
    return isGreater(semverify(lowestImplementedVersion), lowestTargetedVersion.toString());
  });
