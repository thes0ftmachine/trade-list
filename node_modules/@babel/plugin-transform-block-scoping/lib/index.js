...
function wrapLoopBody(loopPath, captured, updatedBindingsUsages) {
  ...
  const fn = types.functionExpression(null, closureParams, types.blockStatement([]));
  ...
  if (labelNum + returnNum === 0) {
    ...
    if (returnNum) {
      for (const path of state.returns) {
        const arg = path.node.argument || types.buildUndefinedNode();
  ...
  const loopBlockBody = types.toBlock(loopNode.body);
...
function isVarInForStatementInit(path) {
...
function transformBlockScopedVariable(path, state, tdzEnabled) {
  ...
  const isNestedForStatementInit = isVarInForStatementInit(path) && isInLoop(path.parentPath);
  const shouldInitLoopDeclaration = isLoopBodyDeclaration || isNestedForStatementInit;
  if (shouldInitLoopDeclaration || dynamicTDZNames.length > 0) {
    for (const decl of path.node.declarations) {
      decl.init ??= types.buildUndefinedNode();
    }
  }
