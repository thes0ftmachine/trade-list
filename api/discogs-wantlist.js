...
// results can be imported into the In Search Of list. Uses the same
// DISCOGS_TOKEN personal access token as discogs-search.js /
// discogs-detail.js, sent purely to get friendlier rate limits — the
// /users/{username}/wants endpoint itself is unauthenticated and public by
// default.
//
  ...
  try {
    while (page <= maxPages) {
      ...
      const discogsRes = await fetch(url, {
        headers: { "User-Agent": "TradeListApp/1.0" },
      });
    ...
    // Normalize each want to look like the shape our own Discogs search
